...
public class CircleCanvasView extends View {
    ...
    protected void onDraw(Canvas canvas) {// 여기에 부채꼴을 그려야함
        ...
        Paint draw_arcStroke = new Paint();
        ...
        if(!todoOrDoneList.isEmpty()) {
            for(PlanInfo p : todoOrDoneList) {
                draw_arcPnt.setColor(Color.parseColor(p.getColor()));
                canvas.drawArc(rect, (float) convertToArcAngle(p.getStart()), (float) angleToMove(p.getStart(),p.getEndAngle()), true, draw_arcPnt);
                canvas.drawArc(rect, (float) convertToArcAngle(p.getStart()), (float) angleToMove(p.getStart(),p.getEndAngle()), true, draw_arcStroke);
